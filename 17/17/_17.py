...
def find_eulerian_path(adj_matrix):
    ...
    if len(odd_degree_nodes) > 2 or (len(odd_degree_nodes) == 1):
        print("graph doesnot have paht or madar oerelry.")
        return None
        
    if not is_connected(adj_matrix):
        print("node not hamdant and path is not oerlery .")
        return None
...
if eulerian_path1:
    print("oelry path found:", " -> ".join(map(str, eulerian_path1)))
print("\n" + "-"*30 + "\n")

# مثال 2: مسیر اویلری (دو گره با درجه فرد)
print("---ex : oerley path---")
adj_matrix2 = [
    [0, 1, 0, 0, 1],
    [1, 0, 1, 0, 0],
    [0, 1, 0, 1, 0],
...
if eulerian_path2:
    print("oelry path:", " -> ".join(map(str, eulerian_path2)))
print("\n" + "-"*30 + "\n")

# مثال 3: بدون مسیر اویلری (سه گره با درجه فرد)
print("--- Example without oelrypath ---")
